...
if (Test-Path $rsrModulePath) {
    ...
    # Fallback to old library location
    $legacyLibPath = Join-Path $PSScriptRoot "..\..\lib\users.ps1"
    if (Test-Path $legacyLibPath) {
        . $legacyLibPath
        Write-Warning "Using legacy library - consider updating to RSR module"
    } else {
