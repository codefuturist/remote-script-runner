...
if [[ -f "$RSR_LIB_DIR/rsr-lib.sh" ]]; then
    source "$RSR_LIB_DIR/rsr-lib.sh" ssh users validate interactive
else
...
# Exit codes (from RSR library)
EXIT_OK=$RSR_EXIT_SUCCESS
EXIT_ERROR=$RSR_EXIT_ERROR
EXIT_INVALID_ARGS=$RSR_EXIT_USAGE
EXIT_PERMISSION=$RSR_EXIT_PERMISSION
EXIT_NOT_INSTALLED=$RSR_EXIT_DEPENDENCY
...
# =============================================================================
# Helper Functions (using RSR library)
# =============================================================================
